...
data_GM12878 <- Read10X(data.dir = "C:/Users/manum/Downloads/GM12878")

# Check the dimensions of the matrix (rows = genes, columns = cells)
dim(data_GM12878)

# Check the first few rows and columns
data_GM12878[1:10, 1:10]

#creating the seurat_object
seurat_object <- CreateSeuratObject(counts = data_GM12878, project = "CancerData", min.cells = 3, min.features = 200)
head(seurat_object)
...
seurat_object <- FindNeighbors(seurat_object, dims = 1:17)
...
seurat_object <- RunUMAP(seurat_object, dims = 1:17)
#individual clusters
