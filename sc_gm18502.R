...
data_GM18502 <- Read10X(data.dir = "C:/Users/manum/Downloads/GSE126321_RAW")

# Check the dimensions of the matrix (rows = genes, columns = cells)
dim(data_GM18502)

# Check the first few rows and columns
data_GM18502[1:10, 1:10]

#creating the seurat_object
seurat_object <- CreateSeuratObject(counts = data_GM18502, project = "CancerData", min.cells = 3, min.features = 200)
head(seurat_object)
...
seurat_object<- FindClusters(seurat_object, resolution = c(0.3, 0.5, 0.7, 1))
View(seurat_object@meta.data)

DimPlot(seurat_object, group.by = "RNA_snn_res.1", label = TRUE)
